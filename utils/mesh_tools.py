...
def extract_data_from_mesh(
    mesh_file: str, points: np.ndarray, origin_data: np.ndarray
) -> np.ndarray:
...
def extract_data_from_mesh1_to_mesh2(
    mesh_file1: str, mesh_file2: str, origin_data: np.ndarray
) -> np.ndarray:
...
def extract_data_from_submesh1_to_submesh2(
    mesh_file1: str, mesh_file2: str, origin_data: np.ndarray, target_subtag: int = 2
) -> np.ndarray:
    domain1, cell_markers_1, _ = gmshio.read_from_msh(
        mesh_file1, MPI.COMM_WORLD, gdim=3
    )
    domain2, cell_markers_2, _ = gmshio.read_from_msh(
        mesh_file2, MPI.COMM_WORLD, gdim=3
    )

    subdomain1, _, _, _ = create_submesh(
        domain1, domain1.topology.dim, cell_markers_1.find(target_subtag)
    )
    subdomain2, _, _, _ = create_submesh(
        domain2, domain2.topology.dim, cell_markers_2.find(target_subtag)
    )
...
def submesh_node_index(
    domain: Mesh, cell_markers: np.ndarray, sub_tag: int
) -> np.ndarray:
    # the idx of submesh node in parent mesh
    tdim = domain.topology.dim
    subdomain, sub_to_parent, _, _ = create_submesh(
        domain, tdim, cell_markers.find(sub_tag)
    )
...
def get_boundary_vertex_connectivity(domain: Mesh):
    ...
    boundary_facets = [f for f in range(num_facets) if len(facet_to_cell.links(f)) == 1]
...
def compute_normal(domain: Mesh):
    tdim = domain.topology.dim
    ...
    b_tag = meshtags(
        ...
        tdim - 1,
        boundary_facets,
        np.array([1] * len(boundary_facets), dtype=np.int32),
    )
