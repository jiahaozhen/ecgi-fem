...
def boundary_condition(u, v, cell_markers, sigma_i=0.4, sigma_e=0.8, sigma_t=0.8):
    ...
    body_boundary_index = locate_entities_boundary(
        domain, tdim - 3, lambda x: np.full(x.shape[1], True, dtype=bool)
    )
    heart_boundary_index = locate_entities_boundary(
    ...
    for i, point in enumerate(heart_boundary_index):
        bc1.append(
            np.dot(grad_u_values_interior[i] + grad_v_values_exterior[i], nh_values[i])
            * sigma_i
        )
        bc2.append(
            np.dot(
                sigma_t * grad_u_values_interior[i]
                - sigma_e * grad_u_values_interior[i],
                nh_values[i],
            )
        )
        bc4.append(
            np.dot(
                (sigma_i + sigma_e - sigma_t) * grad_u_values_interior[i]
                + sigma_i * grad_v_values_exterior[i],
                nh_values[i],
            )
        )
