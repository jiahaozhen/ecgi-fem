...
from utils.mesh_tools import submesh_node_index
...
domain_boundary_index = locate_entities_boundary(
    domain, tdim - 3, lambda x: np.full(x.shape[1], True, dtype=bool)
)
subdomain_boundary_index = locate_entities_boundary(
...
print(
    'error in two ways on get index of submesh' 's point in main mesh:',
    np.linalg.norm(submesh2mesh - submesh2mesh_1),
)
...
print(
    ...
    np.linalg.norm(functionspace_pts_in_order - subdomain_pts[functionspace2mesh]),
)
