...
def process_dataset(input_dir, output_dir, fs=1000):
    ...
    for file_idx, file in enumerate(data_files):
        ...
        try:
            ...
            features = np.transpose(features, (0, 2, 1))  # (B, D, L) -> (B, L, D)
...
def create_features_dataset(
    ...
    process_dataset(
        ...
        output_dir=os.path.join(
            dir_prefix, f"{case_name}/{severity}/d12_features_dataset/"
        ),
    )
    ...
    process_dataset(
        ...
        output_dir=os.path.join(
            dir_prefix, f"{case_name}/{severity}/d64_features_dataset/"
        ),
    )
