...
def label_powerset_classifier(base_classifier=None):
    """
    Label Powerset method considers each unique set of labels that exists in the
    multi-label training data as one class in a multi-class classification problem.
    """
    if base_classifier is None:
        base_classifier = RandomForestClassifier(n_estimators=100, random_state=42)

    classifier = LabelPowerset(classifier=base_classifier, require_dense=[False, True])
    return classifier
...
if __name__ == "__main__":
    ...
    # Test Label Powerset
    print("\n--- Testing Label Powerset ---")
    clf = label_powerset_classifier()
