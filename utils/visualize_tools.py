...
def plot_val_on_mesh(
    ...
    if f_val_flag:
        ...
        plot_val_on_domain(
            domain, val, name=name, tdim=domain.topology.dim, title=title
        )
...
def scatter_on_mesh(mesh_file, pts, gdim=3, target_cell=None, title="Scatter on Mesh"):
    domain, cell_markers, _ = gmshio.read_from_msh(mesh_file, MPI.COMM_WORLD, gdim=gdim)
...
def compare_bsp_on_standard12lead(
    *bsp_datas,
    ...
    standard12Leads = [
        transfer_bsp_to_standard12lead(bsp_data, case_name) for bsp_data in bsp_datas
    ]
    compare_standard_12_lead(
        *standard12Leads,
        labels=labels,
...
def plot_bsp_on_standard12lead(
    bsp_data,
    case_name='normal_male',
    step_per_timeframe=4,
    filter_flag=True,
...
def plot_standard_12_lead(
    standard12Lead, step_per_timeframe=4, filter_flag=True, filter_window_size=50
):
    if filter_flag:
    ...
    time = np.arange(
        0, standard12Lead.shape[0] / step_per_timeframe, 1 / step_per_timeframe
    )
...
def plot_convergence(summary, base_lc=20, base_lc_ratio=1):
    ...
    axs[0].set_title(
        f"Mean Correlation vs lc-lc_ratio (base lc={base_lc}, lc_ratio={base_lc_ratio})"
    )
...
def plot_triangle_mesh(
    ...
    # PyVista cells 格式
    cells = (
        np.hstack([np.c_[np.full(len(triangles), 3), triangles]])
        .ravel()
        .astype(np.int64)
    )
...
def plot_activation_times_on_mesh(
    ...
    gdim=3,
    target_cell=2,
    title="Activation Times on Mesh",
...
def plot_vals_on_mesh(
    ...
    if len(domain.geometry.x) != N:
        raise ValueError(
            f"val_2d 列数 N={N} 与网格点数 {len(domain.geometry.x)} 不一致！"
        )
